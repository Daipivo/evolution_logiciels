...
public class ModuleIdentifier {
     ...
     *
     * @param clusteringHierarchique
     * 		L'ensemble des clusters de l'application.
     * @param weightedGraph
     * 		Le graphe pondéré de l'application.
     * @param CP
     * 		La valeur CP correspond au seuil de couplage entre deux classes.
     * @param M
     * 		La valeur M correspond au nombre de classes de l'application.
     */
    ...
    private Set<Set<String>> identifyModules() {
        ...
        for (Cluster cluster : clusteringHierarchique) {
            ...
            if ((totalPairs == 0) || ((totalCoupling / totalPairs) <= CP))
                continue;
            // Si la moyenne de couplage est inférieure à CP, passez au cluster suivant

            modules.add(classes);
            if (modules.size() >= floor(M / 2.0))
                break;
            // Ne pas dépasser M/2 modules
